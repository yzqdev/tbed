<template>
  ...
  >
    <Drawer
      ...
      placement="top"
      :closable="false"
      v-model="$store.state.noticePopup"
      v-if="
        this.$store.state.metaInfo.explain != '' &&
        this.$store.state.metaInfo.explain != null
      "
    >
      <div
        style="
      ...
      <div
        ...
        "
      >
        <Checkbox style="font-size: 14px" v-model="noticeSwitch"
          >三天内不再通知</Checkbox
        >
    ...
    >
      <Menu
        ...
        style="height: 64px"
        theme="light"
        :active-name="$route.params.activeName"
      >
        <div class="layout-logo" style="height: 100%">
          <img
            ...
            "
            style="height: 100%; margin-left: 35px"
          />
        </div>
        <div class="layout-nav">
          <template v-if="screenWidth <= 568">
            <MenuItem name="ishome" @click.native="homeClick">
              <Icon
                type="md-home"
                style="font-size: 30px; line-height: 2; color: #515a6e"
              />
            </MenuItem>
            <Dropdown
              trigger="click"
            ...
            >
              <a
                ...
                style="color: #515a6e; text-decoration: none"
              >
                <Icon
                  type="md-speedometer"
                  style="font-size: 30px; line-height: 2"
                />&nbsp;
                <Icon type="ios-arrow-down"></Icon>
              </a>
              <DropdownMenu slot="list">
                <template
                  @click.native="controlClick"
                  v-if="this.$store.state.loginStatus"
                >
                  <DropdownItem name="isadmin" @click.native="adminClick"
                    >控制台</DropdownItem
                  >
                  <DropdownItem name="exit" @click.native="exit"
                    >退出</DropdownItem
                  >
                </template>
                <template @click.native="loginClick" v-else>
                  <DropdownItem name="login" @click.native="loginClick"
                    >登录</DropdownItem
                  >
                </template>
                <DropdownItem name="about" @click.native="about"
                  >关于</DropdownItem
                >
              </DropdownMenu>
            </Dropdown>
              ...
              <DropdownMenu slot="list">
                ...
                <template @click.native="loginClick" v-else>
                  <DropdownItem name="login" @click.native="loginClick"
                    >登录</DropdownItem
                  >
                  <DropdownItem name="login" @click.native="registerClick"
                    >注册</DropdownItem
                  >
                </template>
                <DropdownItem name="about" @click.native="about"
    ...
    >
      ...
      <div
        :offset-bottom="10"
        style="position: fixed; right: 55px; bottom: 132px"
        class="animate__animated"
        :class="{ animate__bounce: isShow }"
        v-show="isShow"
      >
    ...
    <Footer
      ...
      v-html="
        this.$store.state.metaInfo.links ? this.$store.state.metaInfo.links : ''
      "
    ></Footer>
    ...
    <Modal v-model="IsAbout" :footer-hide="true">
      ...
      <Card :dis-hover="true" :bordered="false" :shadow="false">
        <div
          ...
          v-html="$store.state.metaInfo.aboutinfo"
        ></div>
      </Card>
    </Modal>
  </Layout>
