...
<template>
  <Layout style="margin-top: 50px">
    ...
    <Content :style="{ margin: '15px 20px 0', minHeight: '500px' }">
      <Row
        :gutter="16"
        v-if="this.$store.state.RoleLevel == 'admin'"
      ...
      >
        ...
        <Col :xs="24" :sm="12" :md="12" :lg="6">
          <Card :shadow="true" class="cardShadow">
            <p slot="title">
              访客容量<Tooltip
                :content="VisitorUpload == 0 ? '访客上传:关' : '访客总容量'"
                theme="light"
        ...
        <Col flex="1" style="padding: 8px">
          <Card class="cardShadow" style="margin-top: 10px">
            <div class="mydiv" style="cursor: pointer" @click="showToken">
              <p>
                <Icon type="md-happy" style="color: #5cdbd3" class="myico" />
              </p>
              <p style="font-size: 16px; line-height: 32px; cursor: pointer">
                {{ helloText }}{{ $store.state.userName }}
              </p>
            </div>
          </Card>
        </Col>
      ...
      <Row
        :gutter="16"
        class="animate__animated animate__fadeIn animate__delay-1.5s cardShadow"
      >
        <Col flex="1">
          <Card :bordered="true" :shadow="true">
            <Row :gutter="16">
              <Col :xs="24" :sm="12" :md="16" :lg="18">
                <div id="fdiv" style="width: 100%">
                  <div style="position: absolute; z-index: 1">
                    <p slot="title">
                      <Dropdown
                        style="margin-left: 15px; border-radius: 50%"
                        trigger="click"
                        v-if="
                          this.$store.state.RoleLevel == 'admin' && ok == 'true'
                        "
                      >
                        <Button size="small">
                          {{ this.viewName }}&nbsp;&nbsp;
                        ...
                        <DropdownMenu slot="list">
                          ...
                          <DropdownItem @click.native="viewType(2)"
                            ><Icon type="ios-podium" /> 全站</DropdownItem
                          >
                        </DropdownMenu>
                      </Dropdown>
                      <Select
              ...
              <Col :xs="24" :sm="12" :md="8" :lg="6">
                <div
                  class="cardstyle"
                  style="height: 350px; width: 100%; padding: 6px"
                >
                  <Tabs size="small">
                    <TabPane label="最近上传" icon="md-clock">
                      ...
                      >
                        <List>
                          <ListItem
                            v-for="(img, index) in Recently.RecentlyUploaded"
                            :key="index"
                          >
                            <ListItemMeta
                              :avatar="img.imgUrl"
                              :title="img.updateTime"
                            />
                          </ListItem>
                        </List>
                      </div>
                    ...
                    >
                      ...
                      >
                        <List>
                          <ListItem
                            v-for="(user, index) in Recently.RecentlyUser"
                            :key="index"
                          >
                            <div v-if="index == 0">
                              <ListItemMeta
...
export default {
  ...
  methods: {
        ...
        .then((res) => {
          if (res.status == 200) {
            ...
            if (res.data.code == "200") {
              ...
              this.ViolationImgTotal = json.ViolationImgTotal
                ? json.ViolationImgTotal
                : 0;
              this.myImgTotal = json.myImgTotal ? json.myImgTotal : 0;
              this.myAlbumTitle = json.myAlbumTitle ? json.myAlbumTitle : 0;
              this.ViolationSwitch = json.ViolationSwitch
                ? json.ViolationSwitch
                : 0;
              this.VisitorProportion = json.VisitorProportion
                ? json.VisitorProportion
                : 0;
    ...
    SpaceExpansion() {
      ...
      request("/admin/SpaceExpansion/" + this.SpaceCode)
        .then((res) => {
          if (res.status == 200) {
            var json = res.data;
            this.isloading = false;
      ...
      request("/admin/getRecently", {})
        .then((res) => {
          if (res.status == 200) {
            ...
            var json = res.data.data;
            if (res.data.code == "200") {
              this.Recently.RecentlyUser = json.RecentlyUser
                ? json.RecentlyUser
                : [];
              this.Recently.RecentlyUploaded = json.RecentlyUploaded
                ? json.RecentlyUploaded
                : [];
            } else {
              this.$Message.error("获取数据失败，请稍后再试");
            }
