  ...
  <Layout style="margin-top: 50px">
    <Content :style="{ margin: '15px 20px 0' }">
      <Tabs>
        <TabPane label="邮箱配置" icon="md-mail">
          ...
          >
            ...
            >
              <FormItem label="邮箱昵称">
                <Input
                  v-model="mailFormItem.emailName"
                  placeholder="邮箱昵称"
                  maxlength="50"
                  size="large"
                ></Input>
              ...
              <FormItem label="端口">
                <Input
                  v-model="mailFormItem.port"
                  type="number"
                  placeholder="端口 如:465"
                  maxlength="10"
                  size="large"
                ></Input>
              </FormItem>
              ...
              >
                <p>提示：</p>
                <p>
                  保存配置前，推荐先进行下方测试，填写一个自己能接收的邮箱地址进行测试，测试无误后再进行开启并保存。
                </p>
                <br />
              </div>
            ...
            >
              ...
              <div v-show="reviewType == 1">
                <FormItem label="APP_ID">
                  <Input
                    v-model="imgReviewFormItem.appId"
                    placeholder="APP_ID"
                    maxlength="500"
                    size="large"
                  ></Input>
                ...
                <FormItem label="API_KEY">
                  <Input
                    v-model="imgReviewFormItem.apiKey"
                    placeholder="API_KEY"
                    maxlength="500"
                    size="large"
                  ></Input>
                ...
                <FormItem label="SECRET_KEY">
                  <Input
                    v-model="imgReviewFormItem.secretKey"
              ...
              >
                <p>注意：</p>
                <p>
                  部分鉴别平台有图像大小或图像格式限制，并非所有图像全部可以鉴别，具体请自行前往对接平台的官方文档查看,并且只能同时开启一个鉴别接口。
                </p>
                <br />
              </div>
              <div style="text-align: center">
                <Button type="primary" shape="circle" @click="SaveConfig(2)"
                  >保 存</Button
                >
              </div>
            </Form>
        ...
        <TabPane label="其他配置" icon="ios-filing" disabled
          >更多功能待开发</TabPane
        >
      </Tabs>
    </Content>
    ...
    <Modal v-model="ismailtest" :footer-hide="true">
      ...
      <Form @submit.native.prevent>
        <FormItem>
          <Input
            prefix="md-mail-open"
            v-model="mailFormItem.toMail"
            placeholder="填待接收邮件的邮箱"
            size="large"
            style="width: 100%"
          />
        </FormItem>
        <div
          style="color: rgb(228 102 70); font-size: 14px; font-weight: 200"
          v-show="iserror"
        >
          <p>异常提示：</p>
          <p>{{ errortext }}</p>
...
export default {
  ...
  methods: {
    getDatas() {
      request("/admin/root/getOrderConfig", {})
        .then((res) => {
          this.$Spin.hide();
          if (res.status == 200) {
    ...
    mailTest() {
      request("/admin/root/mailTest/" + this.mailFormItem.toMail)
        .then((res) => {
          this.$Spin.hide();
          if (res.status == 200) {
            if (res.data.code == "200") {
