  ...
  <Layout style="margin-top: 50px;">
    ...
    <Drawer title="图像类别" :closable="false" v-model="treePopup" :width="screenWidth<=368?screenWidth:368">
      ...
      <Form @submit.native.prevent style="margin-top: 30px;">
        ...
        <FormItem label="起始日期">
          <DatePicker @on-change="startDateChange" format="yyyy-MM-dd HH:mm:ss" type="datetime" split-panels
                      placeholder="起始日期段(默认不限)"></DatePicker>
        </FormItem>
        <FormItem label="结束日期">
          <DatePicker @on-change="stopDateChange" format="yyyy-MM-dd HH:mm:ss" type="datetime" split-panels
                      placeholder="结束日期段(默认当前日期)"></DatePicker>
        </FormItem>
        <FormItem label="用户名">
          ...
          <Col flex="1" v-for="(item,index) in imglist" :key="index">
            <div class="imgdivstyle" :class="[viewType==1?'divimgstyle-max':'divimgstyle-min']">
              ...
              <div class="img-tool-cover" :style="{bottom:toolBottom+ 'px'}">
                <Icon style="cursor:pointer;" @click.native="selectImgs(item)"
                      :type="selectIndex.indexOf(item.id)>-1?'ios-checkmark-circle':'ios-checkmark-circle-outline'"
                      :class="{'icostylecolor' : selectIndex.indexOf(item.id)>-1}" class="icostyle" title="选择"></Icon>
                <Icon style="cursor:pointer;" type="md-link icostyle cobyOrderSn" title="链接"
                      data-clipboard-action="copy" :data-clipboard-text="item.imgUrl" @click.native="copy"/>
                <Icon style="cursor:pointer;" type="md-trash icostyle" @click.native="delImg(item.id,index)"
                      title="删除"></Icon>
                <Icon style="cursor:pointer;" :color="item.violation==null?'':'rgb(228 102 70)'"
                      type="md-information-circle icostyle" @click.native="imgInfo(item)" title="信息"></Icon>
              </div>
            </div>
          </Col>
        </Row>
    ...
    <Modal v-model="isimginfo" :footer-hide="true">
      ...
      <List :split="false">
        <ListItem><span style="text-overflow: ellipsis;white-space: nowrap;overflow: hidden;"><Icon
            style="font-size: 32px;" type="md-image"/>&nbsp;&nbsp;&nbsp;<span
            style="font-size: 18px;">{{ images == null ? '暂缺数据' : images.imgname }}</span></span></ListItem>
      </List>
      ...
      <Tabs>
        <TabPane label="图像信息" icon="ios-images">
          ...
          <div style="line-height: 32px;margin-bottom: 40px;">
            <p><span class="infotitle"> 文件大小：</span><span style="font-size: 14px;">{{
                images == null ? '暂缺数据' : images.sizes > 0 ? this.formatBytes(images.sizes, 2) : ''
              }}</span></p>
            <p><span class="infotitle"> 文件类型：</span><span
                style="font-size: 14px;">{{ images == null ? '未知' : images.format }}</span></p>
            <p><span class="infotitle"> 来源IP：</span><span
                style="font-size: 14px;">{{ images == null ? '暂缺数据' : images.abnormal }}</span></p>
            <p><span class="infotitle"> 上传日期：</span><span style="font-size: 14px;">{{
                images == null ? '暂缺数据' : images.updateTime ? images.updateTime : ''
              }}</span></p>
            ...
            <p><span class="infotitle|"> 存储性质：</span><span
                style="font-size: 14px;">{{ images == null ? '暂缺数据' : (images.imgType == 0 ? '持久' : '暂存') }}</span></p>
          </div>
          <p style="color: rgb(228 102 70);font-size: 12px; font-weight: 200;position: absolute; bottom: 10px; display: block; left: 0; width: 100%;z-index: 1;"
             v-show="isViolation.isControl">{{ isViolation.info }}</p>
    ...
    <AddAlbum :visible ="visible" :album-list="albumList"  @close="closeAddAlbum"></AddAlbum>
    ...
    selectImgs(item) {
      ...
      request(
        ...
        if (res.status == 200) {
          ...
          console.log(`%c这是addto album`,`color:red;font-size:16px;background:transparent`)

          this.albumList = json; console.log(json)
          this.visible = true
        } else {
          this.$Message.error("请求时出现错误");
        }
