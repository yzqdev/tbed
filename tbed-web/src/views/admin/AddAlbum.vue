<template>
  ...
  <Modal :value="visible" @on-visible-change="closeModal" :footer-hide="true" width="620">
    <Tabs>
      <TabPane label="画廊" icon="ios-images">
        <div style="margin:0 auto;max-height: 60vh;overflow-x: hidden;overflow-y: auto;">
          ...
          <Row>
            <Col :xs="24" :sm="24" :md="12" :lg="12"><Input v-model="albumTitle" maxlength="30"
                                                            style="width: 266px;margin: auto;"><span slot="prepend"
        ...
        <Button type="primary" shape="circle" @click.native="submitAlbum" style="float: right;margin-right: 10px;">保存
        </Button>
...
components:{
  ...
  data() {
    return {
      albumTitle: null,
      password: null,isAlbum:false,albumData:{}
    }
  },
  props: {
      ...
      request(
        ...
        if (res.status == 200) {
          ...
          if (res.data.code == '200') {
            ...
            this.albumData.url = window.location.protocol+'//'+window.location.host+'/h/'+json.url;
