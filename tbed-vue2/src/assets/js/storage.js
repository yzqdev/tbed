...
export default {
  ...
  methods: {
    ...
    getKeys() {
      request("/admin/root/getKeysList", {})
        .then((res) => {
          if (res.status == 200) {
    ...
    getLoadInfo() {
      for (let i = 0; i < this.storageList.length; i++) {
        ...
        request(`/admin/root/LoadInfo/${params.keyId}` )
          .then((res) => {
            if (res.status == 200) {
    ...
    save() {
      ...
      request("/admin/root/updateStorage", paramJson)
        .then((res) => {
          this.$Spin.hide();
          if (res.status == 200) {
    ...
    getStorageById(id) {
      this.$Modal.confirm({
        ...
        cancelText: "点错了",
        content:
          "注意！如果把当前存储源修改为其他存储源，将导致当前存储源上传过的所有图像无法删除。",
        onOk: () => {
          // this.$Spin.show();
          this.viewType = "edit";
    ...
    sendGetStorageById(id) {
      ...
      request(`/admin/root/getStorageById/${id}`)
        .then((res) => {
          if (res.status == 200) {
            this.id = id;
            if (res.data.code == "200") {
