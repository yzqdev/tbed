        ...
        <TabPane label="功能配置" icon="ios-images">
          ...
          <Row>
            <Col span="24" style="margin-bottom: 10px">
              <Card shadow>
                <p slot="title">
                  <Icon type="ios-contacts-outline" size="18" /> 访客相关设置
                </p>
                <Row>
                  <Col flex="1">
                    <Form style="padding: 16px">
                      <FormItem label="访客可用总量">
                        ...
                        <Input
                          v-model="uploadConfig.visitormemory"
                          style="width: 130px"
                        >
                  ...
                  <Col flex="1">
                    <Form style="padding: 16px">
                      <FormItem label="图像大小">
                        <Input
                          v-model="uploadConfig.filesizetourists"
                          style="width: 130px"
                        >
                  ...
                  <Col flex="1" style="text-align: center">
                    <Form style="padding: 16px">
                      <FormItem label="上传最多">
                        <Input
                          v-model="uploadConfig.imgcounttourists"
                          style="width: 130px"
                        >
                          <!--                              <span slot="prepend">http://</span>-->
            ...
            <Col span="24" style="margin-bottom: 10px">
              <Card shadow>
                <p slot="title">
                  <Icon type="ios-contacts" size="18" /> 用户相关设置
                </p>
                ...
                <Row>
                  <Col flex="1">
                    <Form style="padding: 16px">
                      <FormItem label="新用户分配">
                        <Input
                          v-model="uploadConfig.usermemory"
                          style="width: 130px"
                        >
                          <span slot="append">Mb</span>
                        </Input>
                  ...
                  <Col flex="1">
                    <Form style="padding: 16px">
                      <FormItem label="图像大小">
                        <Input
                          v-model="uploadConfig.filesizeuser"
                          style="width: 130px"
                        >
                          <!--                              <span slot="prepend">http://</span>-->
                          <span slot="append">Mb</span>
                  ...
                  <Col flex="1" style="text-align: center">
                    <Form style="padding: 16px">
                      <FormItem label="上传最多">
                        <Input
                          v-model="uploadConfig.imgcountuser"
                          style="width: 130px"
                        >
                          <!--                              <span slot="prepend">http://</span>-->
                          <span slot="append">张</span>
              ...
              <Card shadow>
                ...
                <Form style="padding: 16px">
                  <FormItem label="图像格式" :label-width="90">
                    <Input
                      v-model="uploadConfig.suffix"
                      placeholder="请输入图像格式后缀用,隔开(英文逗号)"
                    />
                  </FormItem>
                  <FormItem label="IP黑名单" :label-width="90">
                    <Input
                      v-model="uploadConfig.blacklist"
                      type="textarea"
                      :rows="4"
              ...
              <Card shadow>
                ...
                <Form style="padding: 16px">
                  <FormItem label="站点标题" :label-width="90">
                    <Input
                      v-model="config.webname"
                      maxlength="50"
                      placeholder="请输入网站的标题"
                  ...
                  <FormItem label="后端域名" :label-width="90">
                    <Input
                      ...
                      maxlength="100"
                      placeholder="请输入后端的服务域名或ip:port"
                      style="max-width: 500px"
                    />
                  </FormItem>
                  <FormItem label="站点副标题" :label-width="90">
                    <Input
                      v-model="config.websubtitle"
                      maxlength="100"
                      placeholder="请输入网站的副标题"
                      style="max-width: 500px"
                    />
                  </FormItem>
                  ...
                  <FormItem label="服务端图标" :label-width="90">
                    <Input
                      v-model="config.webfavicons"
                      maxlength="500"
                      placeholder="请输入服务端的favicon图标URL地址"
                    />
                  </FormItem>
                  <FormItem label="站点关键词" :label-width="90">
                    <Input
                      v-model="config.webkeywords"
                      maxlength="500"
                      placeholder="多个关键词用(英文)“ , ”隔开"
                    />
                  ...
                  <FormItem label="统计JS代码" :label-width="90">
                    <Input
                      type="textarea"
                      v-model="config.baidu"
                      :rows="4"
                      maxlength="2000"
                      placeholder="添加网站统计的JS代码，不需要要添加<script>标签</script>"
                    />
                  </FormItem>
...
export default {
  ...
  methods: {
    getDatas() {
      request("/admin/root/getSettingConfig", {})
        .then((res) => {
          this.$Spin.hide();
          if (res.status == 200) {
    ...
    SaveConfig() {
      this.loading = true;
      if (
        this.uploadConfig.suffix == null ||
        this.uploadConfig.suffix.replace(/\s*/g, "") == ""
      ) {
      ...
      if (
        this.config.webname == null ||
        this.config.webname.replace(/\s*/g, "") == ""
      ) {
      ...
      if (
        ...
        this.config.domain.replace(/\s*/g, "") == ""
      ) {
      ...
      if (
        Number(this.uploadConfig.filesizetourists) > 5120 ||
        Number(this.uploadConfig.filesizeuser) > 5120
      ) {
      ...
      if (
        Number(this.uploadConfig.imgcountuser) > 1000 ||
        Number(this.uploadConfig.imgcounttourists) > 1000
      ) {
      ...
      request("/admin/root/updateConfig", params)
        .then((res) => {
          this.loading = false;
          if (res.status == 200) {
            if (res.data.code == "200") {
