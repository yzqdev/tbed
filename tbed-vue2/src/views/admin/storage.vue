    ...
    <Content :style="{ margin: '15px 20px 0' }">
      <Row :gutter="16">
        <Col flex="1" v-for="(sto, index) in storageList" :key="index">
          ...
          >
            <p slot="title" style="display: inline-block; height: 25px">
              <img
                style="max-width: 85px"
                :src="
                  require('../../assets/img/icons/' +
                    sto.storageType.toString() +
                    '.png')
                "
              />
              <Tooltip
                :content="
                  takeEffect(sto.id) || sto.storageType == 5
                    ? '当前状态:可用'
                    : '当前状态:不可用'
                "
                theme="light"
              ...
              >
                <Icon
                  ...
                  size="20"
                  color="#499c54"
                  v-if="takeEffect(sto.id) || sto.storageType == 5"
                />
                <Icon type="md-power" size="20" color="#c75450" v-else />
              </Tooltip>
            </p>
            <div class="stostyle">
              <p>{{ sto.keyname }}</p>
              <p style="position: absolute; bottom: 33px">
                <span>已用空间：</span
                ><span style="font-weight: bold; color: #464c5b">
                  {{ usedCapacity(sto.id) }}
                </span>
              </p>
