<template>
  <div class="layout">
    <Layout :style="{ minHeight: '100vh' }">
      ...
      <Sider
        style="height: 100vh; z-index: 15"
        breakpoint="md"
        collapsible
        :collapsed-width="78"
        v-model="isCollapsed"
        :class="{ miniW: screenWidth <= 768 }"
      >
        <Menu
          :accordion="true"
          active-name="control"
          ...
          style="overflow-y: auto; overflow-x: hidden; height: 90vh"
        >
          <MenuItem name="men01">
            ...
            <img
              @click="goHome"
              :src="
                $store.state.metaInfo.logo == null
                  ? hellohaologo
                  : $store.state.metaInfo.logo
              "
              style="width: 135px"
            />
          </MenuItem>
          <MenuItem
            ...
            @click.native="adminmenuItem('/', null)"
          >
          ...
          <Submenu name="men1">
            ...
            <MenuItem
              name="photo"
              @click.native="adminmenuItem('/photo', '个人中心')"
              >我的相册</MenuItem
            >
            <MenuItem
              name="gallery"
              @click.native="adminmenuItem('/gallery', '个人中心')"
              >画廊管理</MenuItem
            >
          </Submenu>
          ...
          <Submenu name="men5">
            ...
            <MenuItem
              name="photo"
              @click.native="adminmenuItem('/usersetting', '个性设置')"
              >密码修改</MenuItem
            >
            <MenuItem
              v-if="$store.state.metaInfo.watermark == 1"
              name="watermark"
              @click.native="adminmenuItem('/watermark', '个性设置')"
              >水印配置</MenuItem
            >
            <MenuItem
              v-if="$store.state.metaInfo.api == 1"
              name="api"
              @click.native="adminmenuItem('/api', 'API文档')"
              >API文档</MenuItem
            >
          </Submenu>
          ...
          <Submenu name="men2" v-if="$store.state.RoleLevel == 'admin'">
            ...
            <MenuItem
              name="user"
              @click.native="adminmenuItem('/user', '用户管理')"
              >用户列表</MenuItem
            >
            <MenuItem
              name="group"
              @click.native="adminmenuItem('/group', '用户管理')"
              >分发群组</MenuItem
            >
            <MenuItem
              name="capacity"
              @click.native="adminmenuItem('/capacity', '用户管理')"
              >扩容码</MenuItem
            >
          </Submenu>
          ...
          <Submenu name="men3" v-if="$store.state.RoleLevel == 'admin'">
            ...
            <MenuItem
              name="storage"
              @click.native="adminmenuItem('/storage', '系统设置')"
              >存储设置</MenuItem
            >
            <MenuItem
              name="setting"
              @click.native="adminmenuItem('/setting', '系统设置')"
              >站点设置</MenuItem
            >
            <MenuItem
              name="MoreSetting"
              @click.native="adminmenuItem('/moresetting', '系统设置')"
              >更多设置</MenuItem
            >
          </Submenu>
          ...
          <Submenu name="men4" v-if="$store.state.RoleLevel == 'admin'">
            <template slot="title">
              ...
              <Badge dot :offset="wzxy" :count="isshow"
                ><span class="menucolo">更多</span></Badge
              >
            </template>
            <MenuItem
              name="about"
              @click.native="adminmenuItem('/about', '系统设置')"
              ><Badge dot :count="isshow" :offset="wzxy2"
                >关于程序</Badge
              ></MenuItem
            >
          </Submenu>
      ...
      <Layout>
        <div
          ...
          "
          @click="expand"
          v-show="!isCollapsed && screenWidth <= 768"
        ></div>
        <Header
          :style="{
            background: '#fff',
  ...
  metaInfo() {
    return {
      title:
        ...
        this.$store.state.metaInfo.websubtitle,
      meta: [
        {
          name: "keywords",
