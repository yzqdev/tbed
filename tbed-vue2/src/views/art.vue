        ...
        >
          <Form inline @submit.native.prevent>
            ...
            <FormItem style="display: inline-block; width: 80%">
              <Button type="primary" shape="circle" @click="selectPhoto"
                >&nbsp;<Icon type="md-arrow-round-forward" />&nbsp;</Button
              >
            </FormItem>
          </Form>
        </Content>
            ...
            <Col flex="1" v-for="(item, index) in imglist" :key="index">
              <div class="imgdivstyle">
                <img
                  ...
                  :src="item.imgUrl + ''"
                  onerror="this.src='https://www.baidu.com/img/flexible/logo/pc/result.png'"
                />
                <div
...
export default {
  ...
  methods: {
    selectPhoto() {
      ...
      request("/getAlbumList", paramJson)
        .then((res) => {
          console.log(res);
          if (res.status == 200) {
            var json = res.data;
  ...
  mounted() {
    ...
    this.urlKey = window.location.href.substring(
      this.urlKey.indexOf("h/TOALBUM") + 9
    );
    this.getAlbum();
    //this.selectPhoto();
  },
